 ...
 */
export function transform(doc: string, printer: Printer): {svg: string, width: number, height: number} {
	...
	let result = ptr.target.open(ptr);
	// strip bom
	if (doc[0] === '\ufeff') {
		doc = doc.slice(1);
	}
	...
	switch (state.line) {
		...
		case 'horizontal':
			...
			res.push(ptr.target.normal() +
				ptr.target.area(state.rules.left, state.rules.width, state.rules.right) +
				ptr.target.align(0) +
				ptr.target.vrstop(state.rules.widths) +
				ptr.target.vrlf(false));
			state.line = 'waiting';
			break;
		default:
			break;
